...
def main(lr, train_path, eval_path, save_path):
    ...
    preds = clf.predict(x_eval, y_eval)

    plot = plt.plot(y_eval, preds, "ro")
    plt.show()

    np.savetxt(save_path, [y_eval, preds])
...
class PoissonRegression:
    ...
    def fit(self, x, y):
        ...
        for i in range(self.max_iter):
            gradient_step = np.zeros(self.theta.shape)
            for idx, obs in enumerate(x):
                gradient_step += (y[idx] - np.exp(self.theta @ obs)) * obs
            new_theta = self.theta + self.step_size * gradient_step
            if np.linalg.norm(self.theta - new_theta, ord=2) < self.eps:
                break
            ...
            if self.verbose:
                ...
                for idx, res in enumerate(y):
                    loss += (res - np.exp(self.theta @ x[idx])) ** 2
    ...
    def predict(self, x, y):
        ...
        for idx, obs in enumerate(x):
            preds[idx] = np.exp(self.theta @ obs)
